...
#include "fhe/common/primelists.hpp"
#include "fhe/common/rns_transform.hpp"
#include <numeric>
...
const static map<size_t, size_t> std_log_q_size{/*{1024, 27},*/   {2048, 54},
                                                {4096, 109},  {8192, 218},
                                                {16384, 438}/*, {32768, 881}*/};
...
CkksParams create_params(size_t dimension, size_t initial_scaling_bits) {
    ...
    if (log_q_size_it == std_log_q_size.end())
        throw std::string("No suitable primes for this dimension.");

    size_t log_q_size = log_q_size_it->second;
    if (log_q_size < 2 * initial_scaling_bits)
        throw std::string("Initial scaling bits too big.");
    if ((dimension == 2048 && initial_scaling_bits < 19) || initial_scaling_bits < 27)
        throw std::string("Initial scaling bits too small.");

    // printf("%u / %u\n", log_q_size, initial_scaling_bits);
    // if (log_q_size > initial_scaling_bits / 2) {
    //     throw "Initial scaling bits too small.";
    // }
